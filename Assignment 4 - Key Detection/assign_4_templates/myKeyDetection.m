...
if tune == true
    tuneFreq = myTuningFrequencyEstimate(x, blockSize, hopSize, fs);
end

pitchChroma = myPitchChroma(X, fs, tuneFreq);
pitchChroma = mean(pitchChroma, 2);

dist = zeros(2,12);

for i = 0 : 11
    dist(:,i+1) = sqrt(sum((repmat(pitchChroma',2,1)-circshift(template,[0 i])).^2,2));
end

[dist,keyIdx]  = min(dist,[],2);

if (dist(1) < dist(2))
    keyEstimate    = deblank(major(keyIdx(1),:));
else
    keyEstimate    = deblank(minor(keyIdx(2),:));
end 
